...
import projectDefault from '../../assets/images/project-default.svg'
...
// --- the generated half ------------------------------------------------------------
//
// scripts/sync-projects.mjs writes projects.generated.json and one README per
// repository. The cards above stay the source of truth for anything written by hand;
// the generated record only fills what they do not say, and carries the slug that the
// /projects/:slug route and the README file are named after.
//
// A repository that carries the `portfolio` topic and has no card here appears on its
// own, at the end, with whatever GitHub knows about it.

const keyOf = (url) => url.toLowerCase().replace(/\/+$/, '')

const generatedByUrl = new Map(generated.map((g) => [keyOf(g.url), g]))

// Turns "personal-finance-manager" into "Personal Finance Manager", for a repository
// whose README has no title to borrow.
const titleFromSlug = (slug) =>
  slug.replace(/[-_]+/g, ' ').replace(/\b\p{Ll}/gu, (c) => c.toUpperCase())

function fromGenerated(g) {
  return {
    title: g.headline || titleFromSlug(g.name),
    desc: g.description ?? '',
    ...
    pushedAt: g.pushedAt,
    topics: g.topics,
    // The card text is GitHub's own description, which is written in English. Saying so
    // lets the page label it instead of passing it off as a translation.
    autoGenerated: true,
  }
}

// The hand-written value wins whenever there is one, so an empty string in a card never
// silently reopens the generated text.
const pick = (manual, fallback) => (manual === undefined || manual === '' ? fallback : manual)
...
export function mergeProjects() {
  ...
  for (const p of projects) {
    ...
    seen.add(key)
    const g = generatedByUrl.get(key)
    merged.push({
      ...p,
      slug: g?.slug ?? null,
      hasReadme: g?.hasReadme ?? false,
      language: g?.language ?? null,
      pushedAt: g?.pushedAt ?? null,
      topics: g?.topics ?? [],
      desc: pick(p.desc, g?.description ?? ''),
      autoGenerated: false,
    })
  }

  // Appended rather than merged in place, so the six featured cards keep the positions
  // the grid was laid out around and no wide card lands on an odd column.
  for (const g of generated) {
    if (!seen.has(keyOf(g.url))) merged.push(fromGenerated(g))
  }
